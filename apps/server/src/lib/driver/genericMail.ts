...
import { ImapService, ImapConfig, FullMessage as ImapFullMessage } from './imap.service'; // Actual path
import { SmtpService, SmtpConfig } from './smtp.service'; // Actual path
import type { MailManager, IGetThreadResponse, ParsedDraft, IOutgoingMessage, CreateDraftData, Label, ParsedMessage } from './types'; // Actual path
...
export class GenericMailManager implements MailManager {
  ...
  constructor(config: GenericMailManagerConfig) {
    ...
    this.imapService = new ImapService({
        ...
        auth: { // Override auth with top-level credentials
  ...
  async get(id: string): Promise<IGetThreadResponse> {
    ...
    const message = await this.imapService.fetchMessage(mailbox, id);
    if (!message) {
      throw new StandardizedError('Message not found', 404);
    }
    const parsedMessage = this.mapImapMessageToParsedMessage(message, mailbox);
    return {
      messages: [parsedMessage],
      latest: parsedMessage,
      hasUnread: !message.flags.includes('\\Seen'), // IMAP \Seen flag
      totalReplies: 0, // IMAP doesn't inherently track thread replies
      labels: message.flags.map(flag => ({ id: flag, name: flag, type: 'system' })), 
    };
  }
  ...
  async list(params: {
    ...
    const messages = await this.imapService.listMessages(params.folder, params.pageToken as number, params.maxResults);
    ...
    if (messages.length > 0 && params.maxResults && messages.length === params.maxResults) {
        // To determine if there's a next page, we'd ideally fetch one more than maxResults
        // or rely on total message count if available and cheap to get.
        // Simplistic: assume next UID is the token if maxResults were returned.
  ...
  async delete(id: string): Promise<void> {
    ...
    try {
        ...
        console.warn(`Could not move message ${id} to ${trashFolder}. Setting \\Deleted flag as fallback. Expunge not implemented in this manager.`);
        ...
        throw new StandardizedError(`Failed to move message to trash and fallback also problematic: ${error}`, 500, error);
    }
  }
  ...
  async getAttachment(messageId: string, attachmentId: string): Promise<string | undefined> {
    ...
    if (Symbol.asyncIterator in stream) { // Web ReadableStream
        for await (const chunk of stream as any) {
            chunks.push(Buffer.from(chunk));
        }
    } else if (typeof (stream as any).on === 'function') { // Node.js ReadableStream
  ...
  async createDraft(data: CreateDraftData): Promise<{ id?: string | null; success?: boolean; error?: string }> {
    ...
    if (data.cc && data.cc.length > 0) rawMessage += `Cc: ${data.cc.map(t => t.email).join(', ') || ''}\r\n`;
    if (data.bcc && data.bcc.length > 0) rawMessage += `Bcc: ${data.bcc.map(t => t.email).join(', ') || ''}\r\n`; // Note: BCC in headers is unusual for drafts
    ...
    try {
      ...
      // }
      console.warn('createDraft: ImapService.appendMessage method not yet implemented or available. Mocking success.');
  ...
  async getDraft(id: string): Promise<ParsedDraft> {
    ...
    if (!message || !message.flags.includes('\\Draft')) { // Ensure it's a draft
  ...
  async listDrafts(params: { folder?: string, query?: string, maxResults?: number, labelIds?: string[], pageToken?: string | number }): Promise<{ threads: { id: string; $raw?: unknown }[]; nextPageToken: string | null }> {
  ...
  async modifyLabels(ids: string[], options: { addLabels: string[]; removeLabels: string[] }): Promise<void> {
    const mailbox = 'INBOX'; // Placeholder, real mailbox context needed for each id
    
    for (const id of ids) {
        // System flags (non-folder based)
        if (options.addLabels.includes('STARRED')) await this.imapService.setFlags(mailbox, id, ['\\Starred']);
        if (options.addLabels.includes('IMPORTANT')) await this.imapService.setFlags(mailbox, id, ['\\Flagged']); // Standard \Flagged, not "IMPORTANT"
        
        if (options.removeLabels.includes('STARRED')) await this.imapService.unsetFlags(mailbox, id, ['\\Starred']);
        if (options.removeLabels.includes('IMPORTANT')) await this.imapService.unsetFlags(mailbox, id, ['\\Flagged']);
        ...
        } else if (options.removeLabels.includes('INBOX') && !options.addLabels.includes('TRASH')) { // Archive
  ...
  async getUserLabels(): Promise<Label[]> {
    ...
    const systemFlags: Label[] = [ // Represent common IMAP flags as labels
  ...
  async getLabel(id: string): Promise<Label> { // id can be mailbox path or flag name
    // Check if it's a known system flag first
    const systemLabel = (await this.getUserLabels()).find(l => l.id === id && l.type === 'system');
  ...
  async createLabel(label: { name: string; /* color? */ }): Promise<void> {
    ...
    console.warn(`createLabel (createMailbox for ${label.name}) not fully implemented in ImapService for this subtask.`);
  }
  ...
  async updateLabel(id: string, label: { name: string; /* color? */ }): Promise<void> {
    ...
    // await this.imapService.renameMailbox(id, label.name);
     console.warn(`updateLabel (renameMailbox from ${id} to ${label.name}) not fully implemented in ImapService for this subtask.`);
  }
  ...
  async deleteLabel(id: string): Promise<void> {
    ...
    console.warn(`deleteLabel (deleteMailbox ${id}) not fully implemented in ImapService for this subtask.`);
  }

  // --- OAuth Specific (Not Applicable for direct IMAP/SMTP) ---
  async getTokens(code: string): Promise<any> { throw new StandardizedError('OAuth not supported for direct IMAP/SMTP', 501); }
  getScope(): string { return ''; } 
  ...
  async count(): Promise<{ count?: number; label?: string }[]> {
    ...
    for (const mb of mailboxes) {
        try {
            ...
            console.warn(`count for ${mb.name}: ImapService.status method not fully implemented. Returning placeholder.`);
